...
- (void)initData{
    ...
    [_dataArr addObject:@"在UIView的子类方法drawRect 中使用Core Graphics实现绘制蓝色圆"];
    [_dataArr addObject:@"我将在UIView子类的drawLayer:inContext：方法中实现绘图任务，使用UIKit框架。drawLayer:inContext：方法是一个绘制图层内容的代理方法。为了能够调用drawLayer:inContext：方法，我们需要设定图层的代理对象。但要注意，不应该将UIView对象设置为显示层的委托对象，这是因为UIView对象已经是隐式层的代理对象，再将它设置为另一个层的委托对象就会出问题。轻量级的做法是：编写负责绘图形的代理类"];
    [_dataArr addObject:@"在UIView子类的drawLayer:inContext：方法中实现绘图任务，使用CoreGraphics框架"];
    [_dataArr addObject:@"直接使用UIKit框架实现"];
    [_dataArr addObject:@"直接使用CoreGraphic实现"];
    ...
    [_vcNameArr addObject:@"FDrawController"];
    [_vcNameArr addObject:@"FiveController"];
    [_vcNameArr addObject:@"SixController"];
}
